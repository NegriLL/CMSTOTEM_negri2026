...
def main():
    ...
    glue_eta_hist_raw = ROOT.TH1F("glue_eta_raw", "Fake Glueball Pseudorapidity (Raw)", 100, -5, 5)
    glue_eta_hist_cut = ROOT.TH1F("glue_eta_cut", "Fake Glueball Pseudorapidity (Cut)", 100, -5, 5)
    ...
    # "glueball" inv mass reconstruction
    two_rho_mass_hist_raw = ROOT.TH1F("two_rho_mass_raw", "Fake Glueball (Raw)", 100, 1.0, 5.0)
    two_rho_mass_hist_cut = ROOT.TH1F("two_rho_mass_cut", "Fake Glueball (Acceptance)", 100, 1.0, 5.0)
    ...
    for entry in tree:
        ...
        for i in range(entry.ntrk[0]):
            if abs(entry.produced_id[i]) == 211:  # Pion
                ...
                pions.append(vec)
            if abs(entry.produced_id[i]) == 113:  # rho
                px = entry.produced_px[i]
                py = entry.produced_py[i]
                pz = entry.produced_pz[i]
                e = entry.produced_e[i]
        ...
        if len(pions) == 4:
            ...
            mass_loss_diff = mass_loss - two_rho_mass
    ...
    # Formatting fake glueball pseudorapidity
    ...
    save_path = Path(__file__).parent.parent / "plots/rho_plots"
    ...
    # Top right: fake glueball pseudorapidity (raw)
    canvas_eta.cd(2)
    glue_eta_hist_raw.SetLineColor(ROOT.kBlue)
    glue_eta_hist_raw.Draw()
    ROOT.gPad.SetTitle("Fake Glueball (Raw)")
    ...
    # Bottom right: fake glueball pseudorapidity (acceptance cut)
    canvas_eta.cd(4)
    glue_eta_hist_cut.SetLineColor(ROOT.kRed)
    glue_eta_hist_cut.Draw()
    ROOT.gPad.SetTitle("Fake Glueball (Acceptance)")
    ...
    # Mass loss - two rho mass (bar plot for each entry)
    ...
    ax1.set_ylabel("Mass Loss - Two Rho Mass [GeV]")
    ax1.set_title("Mass Loss - Two Rho Mass (Raw)")
    ...
    ax2.set_ylabel("Mass Loss - Two Rho Mass [GeV]")
    ax2.set_title("Mass Loss - Two Rho Mass (Acceptance Cut)")
