...
def pt_together(data, resonant, nonreson, save_path, title):
    ...
    resonant = resonant.Define(
        "pt",
        "sqrt(pow(Sum(produced_px[abs(produced_id) == 211]), 2) + pow(Sum(produced_py[abs(produced_id) == 211]), 2))",
    )
    nonreson = nonreson.Define(
        "pt",
        "sqrt(pow(Sum(produced_px[abs(produced_id) == 211]), 2) + pow(Sum(produced_py[abs(produced_id) == 211]), 2))",
    )
    data_hist = data.Histo1D(("data",) + model[1:], "alltrk_pt")
    resonant_hist = resonant.Histo1D(("reson",) + model[1:], "pt")
    nonreson_hist = nonreson.Histo1D(("nonreson",) + model[1:], "pt")
...
def main():
    ...
    pt_together(data, resonant, nonreson, save_path, title)
